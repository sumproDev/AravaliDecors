...
import type { CmsTestimonial } from "@/lib/cms/public";
    ...
    <section className="section testimonials-section" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      <div className="shell testimonial-shell">
        ...
        <div className="testimonial-controls">
          ...
          <div>{testimonials.map((testimonial, index) => <button key={testimonial.id} type="button" onClick={() => setActive(index)} className={active === index ? "active" : ""} aria-label={`Show testimonial ${index + 1}`} />)}</div>
